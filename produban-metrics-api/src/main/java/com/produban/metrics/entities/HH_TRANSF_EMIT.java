...
public class HH_TRANSF_EMIT implements Serializable, FMetrics, Metrics {
	...
	public void setHh_datos_bancos(HH_DATOS_BANCOS[] hh_datos_bancos) {
		this.hh_datos_bancos = hh_datos_bancos;
	}
	...
	public HH_TRANSF_EMIT(String[] produbanLine, String[] line, String[] fields) {
		...
		this.CODIGO_PRODUCTO = line[FQCaptureMeta.OFFSET_numFields + offset + 3]; // (IMP.PAGO
																					// O
																					// IMP.CONTRAVALOR)
		this.INDICADOR_PETICION = line[FQCaptureMeta.OFFSET_numFields + offset
				+ 8];
		...
		this.CODIGO_MONEDA_IMPORTE_ORDENADO = line[FQCaptureMeta.OFFSET_numFields
				+ offset + 10];
		try {
		...
		this.CODIGO_MONEDA_IMPORTE_CONTRAVALOR = line[FQCaptureMeta.OFFSET_numFields
				+ offset + 12];
		this.GASTOS_EMISION = line[FQCaptureMeta.OFFSET_numFields + offset + 16]; // (O-ORD./B-BENEF/C=COMIS
																					// ORD
																					// GTS
																					// BENEF/G=GTS
																					// ORD
																					// COMIS
																					// BENEF)
		this.INDIC_GASTOS_FUERA_PAIS_EMISOR_POR_CTA = line[FQCaptureMeta.OFFSET_numFields
				+ offset + 17]; // DE: OUR-ORD/BEN-BENEF/SHR-REPARTIR
		...
		this.OFICINA_RECEPTORA = line[FQCaptureMeta.OFFSET_numFields + offset
				+ 28]; // DEL PAGO
		this.CODIGO_PAIS_DESTINO = line[FQCaptureMeta.OFFSET_numFields + offset
				+ 29];
		this.INDICADOR_RESIDENCIA_BENEFICIARIO = line[FQCaptureMeta.OFFSET_numFields
				+ offset + 30];
		this.CODIGO_CANAL = line[FQCaptureMeta.OFFSET_numFields + offset + 32]; // EN
																				// BANCA
																				// ELECTRONICA
		this.CODIGO_USUARIO_ULTIMA_MODIFICACION = line[FQCaptureMeta.OFFSET_numFields
				+ offset + 37];
		try {
			String fecha;
...
*/
	...
	private void createHH_DATOS_BANCOS(String[] produbanLine, String[] line,
		...
		for (int numBanco = 0; numBanco < numBancos; numBanco++) {
			int startLineBanco = FHH_TRANSF_EMIT.LENGTH_HH_TRANSF
					+ (numBanco * FHH_DATOS_BANCOS.LENGTH_HH_DATOS_BANCOS);
			int endLineBanco = startLineBanco
					+ FHH_DATOS_BANCOS.LENGTH_HH_DATOS_BANCOS;
			String[] lineBanco = Arrays.copyOfRange(line, startLineBanco,
					endLineBanco);

			int startExtraFields = FHH_TRANSF_EMIT.LENGTH_EXTRA_FIELDS
					+ (numBanco * FHH_DATOS_BANCOS.LENGTH_EXTRA_FIELDS);
	...
	public String toString() {
		return "HH_TRANSF_EMIT [DATOS_P=" + DATOS_P + ", DATOS_Q=" + DATOS_Q
				...
				+ ", FECHA_ULTIMA_MODIFICACION=" + FECHA_ULTIMA_MODIFICACION
				+ ", hh_datos_bancos=" + Arrays.toString(hh_datos_bancos)
